...
visualisations.WordCloud = function() {
  ...
  stopWordsArr.map(function(d) {
    stopWords.set(d.name);
  });
  ...
  var update = function(duration, settings) {
    if(data) {
      svg
        .style("left", m[1] + "px")
        .style("top", m[0] + "px")
        .attr("width", element.clientWidth = m[1])
        .attr("height", element.clientHeight - m[0]);

      cloud = d3.layout.cloud().size([element.clientWidth - m[1], element.clientHeight - m[0]])
        ...
        .rotate(function() {
          return (~~(Math.random() * 6) - 3) * settings.angle;
        })
        .font(settings.font)
        .fontSize(function(d) {
          return sizes(d.size);
  ...
  function word_count(d) {
    var wordList = d3.map([], function(d) {
      return d;
    });
    var maxCnt = 1;
    ...
    return wordArr.filter(greaterThanX).sort(function(a, b) {
