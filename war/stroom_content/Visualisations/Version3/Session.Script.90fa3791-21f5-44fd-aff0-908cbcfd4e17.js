 ...
 */
/*
 * visualisation to display a day/hour map of sessionised events for multiple series.
...
visualisations.Session = function(containerNode) {
  ...
  var sessioniseInputData = function(data, settings){
    ...
    for (var j = 0; j < seriesCount; j++){
      ...
      for (var i = 0; i < arrLen; i++){
        var startTime = data.values[j].values[i][0];
        var endTime = data.values[j].values[i][1];

        sessionId++;
        sessionisedSeriesData[sessionId] = [];
        sessionisedSeriesData[sessionId][0] = startTime;
        sessionisedSeriesData[sessionId][1] = endTime;
  ...
  this.getInstance = function(containerNode) {
    return new visualisations.Session(containerNode);
  }
